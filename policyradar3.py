...
class PolicyRadarEnhanced:
    ...
    def _get_comprehensive_feeds(self):
        """Return the comprehensive list of verified working URLs from your verification data"""
        return [
    ...
    def scrape_articles_fallback(self, content: str, source_name: str, category: str, url: str) -> List[NewsArticle]:
        ...
        try:
            ...
            # Site-specific selectors based on verified government sources
            site_specific_selectors = {
                "rbi.org.in": ".content-area .news-item, .notification-list li",
                "sebi.gov.in": ".content .press-release, .listing li",
                "meity.gov.in": ".view-content .views-row, .content-area .news-item",
                "trai.gov.in": ".content .views-row, .press-release-list li",
                "pib.gov.in": ".release-content, .content ul li",
                "mea.gov.in": ".content .press-release, .media-briefing",
                "mod.gov.in": ".content .press-release, .news-item",
                "mha.gov.in": ".content .press-release, .whats-new-item",
                "drdo.gov.in": ".content .press-release, .news-item",
                "thehindu.com": ".story-card-33, .story-card",
                "indianexpress.com": ".articles article, .ie-first-story, .article-block",
                "livemint.com": ".cardHolder, .story-list, article",
                "economictimes.indiatimes.com": ".eachStory, .story-card",
                "business-standard.com": ".listing-page, .aticle-list",
            ...
            if not article_elements:
                generic_selectors = [
                    "article, .post, .story-card, .news-item, .card, .press-release",
                    "div.story, div.news, div.article, section.story, .content-item",
                    "div:has(h2) a[href], div:has(h3) a[href], .listing li",
                    ".content a[href], .container a[href], .main-content a[href]"
                ]
            ...
            # Last resort: heading-based approach
            if not article_elements:
                article_elements = []
            ...
            for element in article_elements[:15]:
                ...
                # Process if we have both title and link
    ...
    def assign_tags(self, title, summary):
        ...
        # Enhanced crisis detection for India-Pakistan conflicts
        conflict_keywords = [
            'operation sindoor', 'india-pakistan', 'indo-pak', 'pakistan conflict', 
            'pakistan tension', 'pakistan ceasefire', 'pakistan border', 'pakistan military', 
            'pakistan war', 'loc', 'line of control', 'cross-border', 'border skirmish'
        ]
    ...
    def save_article_to_db(self, article):
        ...
        try:
            ...
            with sqlite3.connect(Config.DB_FILE, timeout=30.0) as conn:
                ...
                c.execute('''REPLACE INTO articles (hash, title, url, source, category, published_date, summary, content, tags, keywords, policy_relevance, source_reliability, recency, sector_specificity, overall_relevance, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                         (article.content_hash, 
                          ...
                          article.content[:5000] if article.content else "", 
                          json.dumps(article.tags)[:1000], 
                          json.dumps(article.keywords)[:1000], 
                          article.relevance_scores.get('policy_relevance', 0), 
                          article.relevance_scores.get('source_reliability', 0), 
                          article.relevance_scores.get('recency', 0), 
                          article.relevance_scores.get('sector_specificity', 0), 
                          article.relevance_scores.get('overall', 0), 
                          json.dumps(article.metadata)[:2000] if hasattr(article, 'metadata') else "{}"))
...
"""
                    ...
                    for article in category_articles[:50]:  # Limit to 50 articles per category
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PolicyRadar Enhanced - System Health</title>
    <style>
...
<body>
    <div class="container">
        <h1>PolicyRadar Enhanced - System Health</h1>
        <p>Last updated: {timestamp}</p>
        
        <div class="status">
            System Status: <span style="color: {status_color}; font-weight: bold;">{system_status}</span>
        </div>
        ...
        <div style="margin-top: 20px; padding: 15px; background: #e8f4f8; border-radius: 5px;">
            <h3>Source Status</h3>
            <p>Monitoring {total_feeds} verified government and news sources across all policy domains.</p>
        </div>
    </div>
</body>
</html>"""
    ...
    def run(self, max_workers: int = 6, args=None) -> str:
        ...
        try:
            ...
            # Log crisis-related articles
            crisis_related = [a for a in all_articles if 'pakistan' in (a.title + a.summary).lower()]
            logger.info(f"Found {len(crisis_related)} articles mentioning Pakistan")
            ...
            # Find crisis articles with tags
            ...
            # Log timestamp verification for crisis articles
            if crisis_articles:
...
if __name__ == "__main__":
    main(){len(articles)}</div><div class="stat-label">Total Articles</div></div>
